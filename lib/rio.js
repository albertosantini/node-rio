...
function parsePacket(data, offset, callback) {
    var res, rt, rl, n = data.length, isWaitingData = waitingData !== null;

    log("Data packet " + n + " bytes - Waiting flag " + isWaitingData);

    if (!isWaitingData) { // not waiting data
        ...
        if (rt === 10) { // packet without header
            rl = int24(data, offset + 1);
            log("rl " + rl + " offset " + offset);

            if ((rl + offset + 4) === n) { // not splitted packet
                ...
                log("Waiting data init");
                waitingData = new Buffer(rl + offset + 4);
                data.copy(waitingData, 0, offset - 4);
                waitingDataLength = n;
                currentWaitingDataLength = n - (offset - 4);
            }
        } else {
        ...
        log("Adding waiting data");
        log("waitingData.length " + waitingData.length);
        log("waitingDataLength " + waitingDataLength);
        log("currentWaitingDataLength " + currentWaitingDataLength);

        data.copy(waitingData, currentWaitingDataLength);
        waitingDataLength += n;
        currentWaitingDataLength += n;

        log("Waiting length " + waitingDataLength +
            " Waiting Data " + waitingData.length);
        if (waitingDataLength === waitingData.length) {
            log("Ending waiting data");

            res = parse_SEXP(waitingData, 4);
...
function dataHandler(data, callback) {
    ...
    } else if (rs === "Rsrv" && n > 32 && !isWaitingData) {
        // packet with protocol header
        parsePacket(data, 32 + 4, callback);
    } else if (n === 16 && int8(data, 0) === 1 && !isWaitingData) {
        // login or header packet
