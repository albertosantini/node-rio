...
function evaluate(cmd, options) {
    ...
    binary(conn)
        ...
        .tap(function (vars) {
            ...
            if (vars.rServeProtocolVersion.toString('ascii') !== "0103") {
                log("Unsupported protocol version " + vars.rServeIdSignature);
            } else {
                log("Supported protocol version 0103");

                cap = vars.rServeCapabilities.toString('ascii');
        ...
        .tap(function (vars) {
            ...
            this.buffer("dataPacket", rl)
                .tap(function (vars) {
                    var res = parse_SEXP(vars.dataPacket, 0);
                    log(vars.dataPacket);
