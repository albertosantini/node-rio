...
function sendAction(cmd, type, msg, options) {
    ...
    var callback=function(err, res){  //callback wrapper to avoid dups
        if(!callbacked) cb(err, res);
        callbacked=true;
    ...
    binary(conn)
        ...
        .tap(function (vars) {
            ...
            if (rs !== "Rsrv" && rv !== "0103") {
                var errorString="Unsupported protocol " + rs + " " + rv;
                log(errorString);
                ...
                if (CAPABILITIES.PLAIN) {
                    this.buffer("loginResponse", 16)
                        .tap(function (vars) {
                            ...
                            if (rr !== 1) {
                                var errorString="Response with error code " + sc;
                                log(errorString);
                                callback(errorString);
                                close(conn);
                            }
        ...
        .tap(function (vars) {
            ...
            if ((cmdResponse & 255) !== 1) {
                var errorString="Eval failed with error code " +
                    ((cmdResponse >> 24) & 127);
                log(errorString);
                callback(errorString);
        ...
        .tap(function (vars) {
            ...
            if (responseType !== 10) {
                var errorString="Invalid response (expecting SEXP)";
                log(errorString);
                callback(errorString);
                close(conn);
            }
